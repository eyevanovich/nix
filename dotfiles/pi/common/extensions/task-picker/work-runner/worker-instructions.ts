...
- Immediately before validation, repeat \`git ls-remote --exit-code origin HEAD\`. Resolve an authentication wait exactly as above; do not start a no-mistakes run that is already unable to fetch the trusted default branch resolved from authoritative remote metadata.
