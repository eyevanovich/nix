...
1. **File issues for remaining work** - Create local Beads issues for anything that needs follow-up
...
local-only in this public repository:

- Never stage or commit `.beads/`.
- Never sync Beads to the public Git origin.
- Use a separate private Dolt remote if cross-machine issue sync is needed.
